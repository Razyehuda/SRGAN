...
class SRGANTrainer:
        ...
        for batch_idx, (lr_imgs, hr_imgs) in enumerate(progress_bar):
            ...
            if pretrain:
                ...
                for _ in range(self.config.g_steps):
                    self.g_optimizer.zero_grad()
                    fake_imgs = self.generator(lr_imgs)
                    fake_outputs = self.discriminator(fake_imgs.clamp(-1.0, 1.0))
    ...
    def save_samples(self, epoch):
        ...
        with torch.no_grad():
            ...
            for i in range(num_samples):
                # Low resolution
                axes[i, 0].imshow(np.transpose(lr_imgs_np[i], (1, 2, 0)))
                axes[i, 0].set_title('Low Resolution')
                axes[i, 0].axis('off')
                
                # Super resolved
                axes[i, 1].imshow(np.transpose(sr_imgs_np[i], (1, 2, 0)))
                axes[i, 1].set_title('Super Resolved')
                axes[i, 1].axis('off')
                
                # High resolution
                axes[i, 2].imshow(np.transpose(hr_imgs_np[i], (1, 2, 0)))
    ...
    def save_loss_data(self):
        ...
        with open(csv_path, 'w', newline='') as csvfile:
            fieldnames = ['epoch', 'g_loss', 'd_loss', 'content_loss', 'perceptual_loss', 'adversarial_loss', 'psnr', 'ssim']
...
def main():
    ...
    parser.add_argument('--d_lr', type=float, default=1e-6, help='Discriminator learning rate')
    ...
    parser.add_argument('--adversarial_weight', type=float, default=1e-6, help='Adversarial loss weight')

#!/usr/bin/env python3
"""
Script to create training plots from TensorBoard logs for the original SRGAN model.
This script extracts training data from the logs directory and creates
...
def create_training_plots(data, output_dir='training_plots_original'):
    """Create comprehensive training plots for the original model."""
    ...
    fig.suptitle('Original SRGAN Training Progress', fontsize=16, fontweight='bold')
    
    # Plot 1: Generator Loss
    if 'Loss/Generator' in data:
        ax = axes[0, 0]
        ax.plot(data['Loss/Generator']['steps'], data['Loss/Generator']['values'], 
                label='Generator', color='blue', linewidth=2, marker='o', markersize=4)
    ...
    if 'Loss/Discriminator' in data:
        ax = axes[0, 1]
        ax.plot(data['Loss/Discriminator']['steps'], data['Loss/Discriminator']['values'], 
    ...
    if 'Loss/Generator' in data:
        final_g_loss = data['Loss/Generator']['values'][-1]
        summary_text.append(f'Final G Loss: {final_g_loss:.4f}')
    
    if 'Loss/Discriminator' in data:
        final_d_loss = data['Loss/Discriminator']['values'][-1]
...
def save_data_to_csv(data, output_dir):
    ...
    if 'Metrics/PSNR' in all_data and 'Metrics/SSIM' in all_data:
        ...
        if 'Loss_Generator' in all_data:
            combined_df['generator_loss'] = all_data['Loss_Generator']['value']
        if 'Loss_Discriminator' in all_data:
            combined_df['discriminator_loss'] = all_data['Loss_Discriminator']['value']
...
def main():
    """Main function to create training plots for the original model."""
    print("Original SRGAN Training Plot Generator")
    print("=" * 40)
    ...
    if data:
        ...
        print(f"Total training steps: {len(data.get('Metrics/PSNR', {}).get('steps', []))}")
        print("=" * 40)
    else:
        print("❌ No training data found!")
        print("Make sure you have completed training and have TensorBoard logs in the logs directory.")
